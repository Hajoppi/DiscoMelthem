...
Shader "DiscoMeltem/Basic_shader" {
    Properties {
        ...
        _Emission ("Emission", Range(1, 11)) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float3 finalColor = emissive + (((node_9873*node_7782)+(node_7782*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_Gloss)))*_LightColor0.rgb*_SpecPower))*_LightColor0.rgb*attenuation*_Emission);
...
////// Lighting:
                ...
                float3 node_9873 = saturate((lerp(1.0,i.posWorld.g,_GradientPower)*(_BasicTexture_var.rgb*lerp(_ColdColor.rgb,_HotColor.rgb,_Heat))));
                float node_7782 = max(0,dot(lightDirection,normalDirection)); // Lambert
                float3 finalColor = (((node_9873*node_7782)+(node_7782*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_Gloss)))*_LightColor0.rgb*_SpecPower))*_LightColor0.rgb*attenuation*_Emission);
